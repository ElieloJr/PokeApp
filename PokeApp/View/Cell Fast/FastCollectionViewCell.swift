...
class FastCollectionViewCell: UICollectionViewCell {
    ...
    @IBOutlet weak var backgroundFastView: UIView!
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        backgroundFastView.layer.cornerRadius = 20
    }
    func setData(pokemon: RecentPokemonList) {
        guard let name = pokemon.name else { return }
        guard let id = pokemon.id else { return }
        guard let urlImage = pokemon.urlImage else { return }
        guard let type = PokemonTypes(rawValue: pokemon.type!) else { return }
        
        pokemonNameLabel.text = name.capitalized
        pokemonIdLabel.text = "#\(id)"
        backgroundFastView.backgroundColor = getColorFor(type: type)
        ...
        DispatchQueue.global().async {
            ...
            DispatchQueue.main.async {
                self.pokemonImage.image = imageEnd
            }
        }
    }
